...
Besides the Pearson's R, an appropriate statistic to report in relability studies is the intraclass correlation (ICC), a measure of association when studying the reliability of raters. There are several formulations for the ICC whose choice may depend on the particular experimental design. According to the seminal work by @shrout1979, these are:
...
Additionally, the ICC may depend on whether one consider the ratings are the average of not of several rating. When averaged is considered, these are called ICC(1,k), ICC(2,k) and ICC(3,k).

The ICC can be computed using the `ICC()` or the `icc()` functions of the *psych* or *irr* packages, respectively. Let's play first with the former using the dataframe where estimates by the raters are shown in each column.
