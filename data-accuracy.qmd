...
```{r}
#| label: tbl-ccc
#| tbl-cap: "Statitics of the concordance correlation coefficient summarizing accuracy and precision of visual severity estimates of soybean rust for four raters"
...
Incidence data are binary at the individual level; an individual is diseased or not. We can simulate two series of binary data and assign them to estimate and true values. 
...
estimate <- rbinom(100, size = 1, prob = 0.6)
actual <- rbinom(100, size = 1, prob = 0.6)

library(caret)
confusionMatrix(as.factor(estimate),as.factor(actual))

```


### MixtureMissing package

```{r}
#| warning: false
library(MixtureMissing)
evaluation_metrics(estimate, actual)
```
