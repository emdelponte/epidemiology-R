...
According to @shah2019a, this method offers several advantages, particularly when classifying epidemics into binary categories (epidemic vs. non-epidemic). First, it can reveal how weather variables differ between epidemic and non-epidemic periods over extended times, providing a more nuanced understanding of potential disease drivers. By maintaining data continuity, FDA can detect early signs of divergence in weather patterns that may precede disease outbreaks. Additionally, FDA minimizes the arbitrary segmentations inherent in the window-pane method, leading to more stable and robust detection of signals relevant to epidemic risk. This continuous representation aligns better with the gradual, overlapping nature of many plant-pathogen interactions, offering clearer insights into the environmental conditions that promote disease development.
...
# Create the dataset based on your code
dat_T2M <- trials_weather |> 
  mutate(epidemic = case_when(
    inc_mean <= 25 ~ 0,
    inc_mean > 25  ~ 1)) |> 
  select(study, T2M, YYYYMMDD, heading, epidemic) |>
  mutate(days = as.numeric(-(heading - YYYYMMDD))) |> 
  select(study, T2M, days, epidemic)

# Transform to wide format to create a matrix of T2M values
dat_wide <- dat_T2M |>
  pivot_wider(names_from = days, values_from = T2M)

# Extract time points and clean column names to ensure numeric conversion
time_points <- as.numeric(gsub("[^0-9.-]", "", colnames(dat_wide)[-1]))

# Ensure there are no NA values in time_points
time_points <- time_points[!is.na(time_points)]

# Create a matrix for T2M values
T2M_matrix <- as.matrix(dat_wide[,-1])  # Exclude the 'study' column, keep only T2M values

# Check dimensions
if (length(time_points) != ncol(T2M_matrix)) {
  stop("Mismatch between number of time points and number of columns in T2M_matrix")
}

# Create the data list for pffr
dat <- list(
  y = dat_T2M$epidemic,   # Response variable
  x = T2M_matrix,         # Matrix of predictors
  yind = time_points      # Cleaned time indices
)
...
m2 <- pffr(y ~ x, 
           ...
           algorithm = "bam")  # Use 'bam' as the default fitting algorithm
