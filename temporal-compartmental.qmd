...
![Schematic showing the H-L-I-R compartmental model. The arrows represent the three transitions tracked by the model: infection, emergence of infectivity and loss of infectivity. The dashed line linking the $I$ compartment to the infection transition indicates the rate of this transition depends on the value of $I$ (as well as the value of $H$).](imgs/HLIR%20Schematic.png){fig-align="center" width="75%"}
...
plot.model.results <- function(data, type)
{
  ...
  if(type == "HLIR" | type == "HLIRP" | type == "HLIRP (with host growth)")
  {
    ...
    if(type == "HLIRP (with host growth)")
    {
      ...
      gather_cols <- c("H", "L", "I", "R", "D", "T_tissue")
      ...
      gather_cols <- c("H", "L", "I", "R", "D")
      legend_pos <- c(0.15, 0.5)
    }
    
    plot_long <- plot_df %>%
      select(time, all_of(gather_cols)) %>%
      pivot_longer(cols = -time, names_to = "Compartment", values_to = "Value") %>%
      mutate(Compartment = factor(Compartment, levels = gather_cols))
    
    p <- ggplot(plot_long, aes(x = time, y = Value, color = Compartment, linetype = Compartment)) +
      geom_line(linewidth = 0.8) +
      scale_color_manual(values = cols, labels = labels) +
      scale_linetype_manual(values = ltys, labels = labels) +
      labs(
        x = "Time (days)",
        y = y.lab,
        title = paste(type, "model"),
        color = NULL,
        linetype = NULL
      ) +
  ...
  if(type == "MULTI")
  {
    ...
    {
      res <- data$results[[i]]
      t_years <- ((i-1)*365 + res$time) / 365
      multi_list[[i]] <- data.frame(
        time = t_years,
        D = res$L + res$I + res$R,
        T_tissue = res$H + res$L + res$I + res$R,
        Season = factor(i)
      )
      breaks <- c(breaks, max(t_years))
    }
    multi_plot_df <- bind_rows(multi_list)
    
    multi_long <- multi_plot_df %>%
      pivot_longer(cols = c(D, T_tissue), names_to = "Variable", values_to = "Value")
    
    p1 <- ggplot(multi_long, aes(x = time, y = Value, color = Variable, group = interaction(Season, Variable))) +
      geom_line(linewidth = 0.8) +
      geom_vline(xintercept = breaks, color = "grey", linetype = "dashed", linewidth = 0.8) +
      scale_color_manual(
        values = c("D" = "black", "T_tissue" = "dodgerblue"),
        labels = c("D" = "(D)iseased", "T_tissue" = "(T)otal")
      ) +
      labs(
        x = "Time (years)",
        y = "Amount of host tissue",
        title = "Multi-seasonal epidemic. Disease progress",
        color = NULL
      ) +
      ylim(0, 1.15*data$n.max) +
    ...
    inoc_df <- data.frame(
      Year = 1:data$n.yr,
      log_Inoc = log(data$inoc)
    )
    p2 <- ggplot(inoc_df, aes(x = Year, y = log_Inoc)) +
      geom_line(linewidth = 0.8) +
      geom_point(size = 3) +
      labs(
        x = "Time (years)",
        y = "log(Inoculum) at start of season",
        title = "Multi-seasonal epidemic. log(Inoculum) level"
      ) +
      scale_x_continuous(breaks = 1:data$n.yr) +
      theme_r4pde()
...
The number of diseased hosts, $D(t)$, is also shown, where \begin{align}
D(t) &= L(t) + I(t) + R(t).
\end{align}
...
Considering only the infections directly caused by the single infected host and multiplying the net rate at which healthy hosts become infected by the host's average infectious period implies \begin{align}
R_0 &= \mbox{Rate of infection per healthy host} \times \mbox{Number of healthy hosts} \times \mbox{Infectious period},\\
    ...
    &= \frac{\beta N}{\mu}.
\end{align} This is because the infected host generates infections at total rate $\beta N$ and remains infectious for $1/\mu$ time units.
...
This initial exponential rate of disease increase can be found [@Cunniffe2012] by calculating the largest real part of the eigenvalues of the Jacobian matrix of the system evaluated at the disease-free-equilibrium $$(H,L,I,R)=(N,0,0,0)$$ The Jacobian matrix is simply a mathematical object that captures how small perturbations to each compartment affect the rates of change of all other compartments.
...
p2 <- ggplot(subset_df, aes(x = time, y = log_I)) +
  geom_line(linewidth = 0.8) +
...
$$\begin{aligned}
 ...
 \frac{dR}{dt} &= \mu I,\\
 \frac{dP}{dt} &= -\omega P.
\end{aligned}$$ This model can be simulated too.
...
There are various ways of modelling host growth. We introduce the idea in a simple way here, with logistic growth of healthy hosts at rate $r_H$, to a carrying capacity $N_{max}$. Adding this to the H-L-I-R-(P) model leads to $$\begin{aligned}
...
p <- ggplot(ss_df, aes(x = beta, y = ss)) +
  ...
  geom_vline(aes(xintercept = fitted_beta, color = "Fitted beta"), linetype = "dashed", linewidth = 0.8) +
...
p <- ggplot() +
  ...
  geom_line(data = orig.beta.dpc, aes(x = time, y = D, color = "Original parameters"), linetype = "dashed", linewidth = 0.8) +
  scale_color_manual(
    values = c(
      "Observations" = "red",
      "Fitted model" = "black",
...
Assume that the latent period is infinitesimally short (i.e., $\gamma \rightarrow \infty$) and that infectious tissue does not lose infectivity (i.e., $\mu = 0$). This simplifies the dynamics $$\begin{aligned}
 \frac{dH}{dt} &= -\left(\epsilon + \beta I\right) H,\\
 \frac{dI}{dt} &= \left(\epsilon + \beta I\right) H.\\
\end{aligned}$$ But then, since $H + I = N$, the model reduces down to a single equation for the number of infected plants $$ \frac{dI}{dt} = \left(\epsilon +\beta I\right)(N-I).\\$$ As a historical aside, note this form of model was introduced by @Brassett1988 as a "differential equation that combines the properties of the monomolecular and logistic models", and used to understand take-all disease in wheat. Here we have seen how it can be derived from the H-L-I-R model.
...
A concordance between process-based models and the Gompertz model can be made if we move to a more complex Kermack-McMcKermack type model in which the infection rate depends on the time-since-infection [@Segarra2001; @Cunniffe2012], at least approximately. However, this is really quite advanced and requires going well beyond what can be covered in this chapter, so we do not pursue this here.
