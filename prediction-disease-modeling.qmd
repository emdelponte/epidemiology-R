...
df_epidemic <- trials_weather |> 
  mutate(epidemic = if_else(inc_mean > 20, 1, 0),
           days = as.numeric(-(heading - YYYYMMDD))) |>
  filter(epidemic == 1) |> 
  dplyr::select(T2M_MIN, days, study)
...
df_epidemic_wide <- df_epidemic |>
  group_by(study) |>
  pivot_wider(names_from = days, values_from = T2M_MIN) |>
  ungroup() |>
  dplyr::select(-study)
...
# Perform FDA test
itp_result <- ITP2bspline(data1 = data_epidemic, data2 = data_non_epidemic, B = 100)
...
# Plot FDA results
plot(itp_result, main = "T2M_MIN", xrange = c(-28, 28), xlab = 'Day', xaxt = 'n')
